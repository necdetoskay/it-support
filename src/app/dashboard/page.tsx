...
export default function KontrolPaneliSayfasi() {
  ...
  useEffect(() => {
    const yetkiKontrol = () => {
      ...
      if (!token || !kullaniciVerisi) {
        ...
        yonlendirici.replace("/auth/login");
        return;
      }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <button 
            onClick={() => yonlendirici.replace("/auth/login")}
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-4">Hoş Geldiniz, {kullanici?.name}</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {/* Karşılama Kartı */}
        <Card className="p-6 col-span-full bg-white">
          <h2 className="text-xl font-semibold text-gray-800">
            Hoş geldin, {kullanici.name}!
          </h2>
