...
interface KategoriModalYeniProps {
  open: boolean;
...
export function KategoriModalYeni({
  open,
  onOpenChange,
  kategoriId,
  onSuccess,
}: KategoriModalYeniProps) {
  ...
  const loadKategori = async () => {
    try {
      const response = await fetch(`/api/kategoriler/${kategoriId}`);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {kategoriId ? "Kategori Düzenle" : "Yeni Kategori"}
          </DialogTitle>
        </DialogHeader>
        ...
        {formLoading ? (
          <div className="py-8">
            <Spinner size="lg" />
            <p className="text-center mt-2 text-sm text-muted-foreground">Kategori bilgileri yükleniyor...</p>
          </div>
        ) : (
